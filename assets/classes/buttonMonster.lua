--[[ *** ButtonMonster ***
	A Monster Button Class with Text, Pixel, Images 9patch (Up, Down, Disabled), Tooltip and Sfx
...
function ButtonMonster:init(xparams, xselector)
	...
	self.params.imagepaddingx = xparams.imagepaddingx or 32 -- number
	self.params.imagepaddingy = xparams.imagepaddingy or self.params.imagepaddingx -- number
	-- text?
		...
		for k, v in ipairs(self.btns) do
			...
			elseif v.selector == v:getParent().selector then -- button is focused (down state)
				if v.params.hover then -- can hover option
					if v.params.scalexdown ~= nil then v:setScale(v.params.scalexdown, v.params.scaleydown) end
					if v.params.imgup ~= nil then v.bmpup:setVisible(false) end
				...
				if v.params.scalexup ~= nil then v:setScale(v.params.scalexup, v.params.scaleyup) end
